...
2. **Tarjetas de sellos** — un tipo de tarjeta alterno que muestra el conteo como texto ("7 de 10 sellos", no una grilla visual de íconos — ver §4.2 sobre por qué se descartó la imagen compuesta), reutilizando la misma columna `puntos_actuales` como contador.
...
**Completar la tarjeta — segunda corrección, más importante que la primera: NO "ya funciona hoy."** La versión anterior de este párrafo afirmaba que el canje "ya existe... sin ningún código nuevo... por el mismo camino que ya funciona hoy." **Es falso, y la segunda revisión lo comprobó de raíz:** no existe ningún código de canje en todo el proyecto. Un grep de `canje|puntos_gastados|.rpc(` sobre todo `lib/` y `app/` no encuentra más que la definición de la tabla `canjes` — cero inserts, cero función que reste `puntos_actuales`. El único código que toca esa columna hoy (`app/api/tarjetas/[tarjetaId]/puntos/route.ts`) **solo suma** y rechaza explícitamente cualquier delta negativo. O sea que reemplacé un pipeline de imágenes sin construir por un mecanismo de canje igual de sin construir, y lo describí como si ya funcionara.

**Lo honesto:** qué pasa al llegar a `sello_meta` —igual que qué pasa al canjear cualquier recompensa por puntos— es trabajo de la Fase 4 (cajero + canje), que ya estaba fuera de alcance de esta fase (§9) antes de este documento. Esta fase construye el contador y su texto ("7 de 10"); no construye la redención de ninguno de los dos tipos de tarjeta. La pregunta de "qué pasa si `puntos_actuales >= sello_meta`" no está "resuelta de raíz" — queda diferida, igual que el canje de puntos siempre lo estuvo, y ninguna de las dos cosas se rompe por eso: nada de lo que se construye en esta fase depende de que el canje ya exista.
