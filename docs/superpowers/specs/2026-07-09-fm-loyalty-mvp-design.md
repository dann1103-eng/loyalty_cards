...
- Rate limiting: diferido a Fase 4+ — el registro público sin límite de tasa es aceptable en el piloto (sin amplificación de costo: no SMS ni push al registrar), documentado como deuda consciente.
- El endpoint de sumar puntos del walking skeleton se expone SIN autenticación solo para la verificación técnica (Tarea 12); DEBE ganar autenticación de cajero (Fase 4) antes de cualquier uso real — ya estaba en el plan (§ simplificaciones), se registra también aquí como decisión.
