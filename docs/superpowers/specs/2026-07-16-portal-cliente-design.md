...
- Instalable: `manifest` (ícono, nombre, color de tema). Sin service worker en esta primera versión — no hay necesidad de soporte offline para una consulta de saldo. **Matiz importante dado que todos los clientes reales están en iPhone (ver §4):** en Android/Chrome de escritorio el navegador sí puede ofrecer "agregar a pantalla de inicio" proactivamente; en iOS Safari es una acción manual desde Compartir, no algo que el navegador ofrezca solo.
...
**Mitigación (gratuita, sin servicios de terceros), corregida tras una segunda revisión — el primer intento tenía dos fallas reales:**

1. **Justifiqué la ruta API con una razón falsa.** Dije que hacía falta porque "un Server Action no expone la IP tan directo" — no es cierto en Next 16: `headers()` de `next/headers` funciona igual dentro de un Server Action. La ruta API (`app/api/portal/consulta/route.ts`, POST) se mantiene, pero por una razón real: consistencia con el endpoint hermano ya existente (`/api/tarjetas/[id]/puntos`), no una necesidad técnica inventada.
2. **La mitigación en sí quedaba rota.** Proponía leer `x-forwarded-for`/`x-real-ip` a mano — en Vercel eso es **falsificable**: Vercel *añade* la IP real a la cadena en vez de reemplazarla, así que un lector ingenuo que toma el primer valor lee lo que el atacante quiera, y rotar esa cabecera en cada request habría anulado el límite por completo — exactamente el ataque que esto existe para frenar.

**Diseño corregido:** tabla nueva `intentos_consulta_portal(id, ip, created_at)` con índice en `(ip, created_at)`. Cada intento (exitoso o no) inserta una fila. La IP se obtiene con el paquete oficial `@vercel/functions` (exporta un helper para esto, pensado para resolver la cadena de proxies de Vercel correctamente en vez de parsear cabeceras a mano) — **la tarea de implementación debe confirmar el nombre exacto de la función y su comportamiento contra el paquete instalado antes de escribir el código**, no asumirlo de memoria; si por algún motivo no sirviera, la alternativa documentada de Vercel es tomar el **último** valor de la cadena `x-forwarded-for` (Vercel añade el suyo al final, no lo reemplaza), nunca el primero. Antes de cada consulta, contar intentos de esa IP en los últimos 15 minutos; más de 10 → responder "Demasiados intentos, intenta de nuevo más tarde" sin tocar `clientes`.
...
- **Ruta API** `app/api/portal/consulta/route.ts` (POST, no Server Action — por consistencia con el endpoint hermano `/api/tarjetas/[id]/puntos`, no porque un Server Action no pudiera leer la IP: sí puede, con `headers()` de `next/headers`). Recibe el teléfono, aplica el límite de intentos (§3: `ipAddress()` de `@vercel/functions`, tabla `intentos_consulta_portal`), busca en `clientes` por `telefono`, y devuelve las `tarjetas` asociadas con su comercio (nombre, colores, tipo de tarjeta, saldo) y las `recompensas` activas de cada comercio. Usa `createServiceClient()`.
- Página cliente (`'use client'`) con un formulario simple de teléfono → resultado, consumiendo esa ruta.
- `app/manifest.ts` (soportado nativamente por Next.js: `MetadataRoute.Manifest`) + **íconos reales de 192×192 y 512×512 en `public/`** (hoy `public/` solo tiene los SVG de arranque de Next — no sirven para esto, hace falta generarlos/exportarlos del branding genérico de FM), más un **`apple-touch-icon` de 180×180** (tamaño recomendado por Apple para el ícono de inicio en iOS, distinto de los del manifest).
- **Honestidad sobre iOS, ya que todos los clientes reales están en iPhone:** Safari **no ofrece** proactivamente "agregar a inicio" — es una acción manual desde el botón de Compartir. El ícono de inicio en iOS no sale del `manifest`, sale de la etiqueta `<link rel="apple-touch-icon">` de 180×180 (y metadata `appleWebApp`) que hay que agregar al layout de esta ruta — el layout raíz hoy (`app/layout.tsx`) no la tiene. El copy de la página debe decir "agrégala desde Compartir → Agregar a inicio", no insinuar que el navegador la va a ofrecer solo.
