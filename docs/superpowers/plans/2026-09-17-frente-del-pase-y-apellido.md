...
- Si un `git commit` choca con `index.lock` (otro subagente commiteando a la vez), esperá unos segundos y reintentá; nunca borres el lock.
- **Nadie toca `app/globals.css`** en esta entrega.
...
- [ ] **`PIE_CODIGO`**: constante exportada con el literal `'Powered by Cardly'`. NO la armes con `MARCA.nombre` de `lib/marca.ts`: vale `'Cardly SV'` y el diseño dice "Cardly".
...
- [ ] **Ruta `/api/registro`** (no tiene prueba hoy): la validación de nombre Y apellido sale a una función pura nueva `validarNombreRegistro({ nombre, apellido })` en `lib/clientes/validarNombreRegistro.ts` (+test), que devuelve `{ ok: true, nombre, apellido }` recortados o `{ ok: false, error }`, y la ruta la usa. Espejo exacto para los dos: `typeof x !== 'string' || !x` → `'Faltan datos'`; recortado vacío o `> 120` → `'Nombre inválido'` / `'Apellido inválido'`. La ruta sigue respondiendo 400 con ese error. Pruebas de cada caso y MUTACIÓN (tope 121 aceptado).
- [ ] **`RegistroCliente.tsx`**: estado `apellido`; campo "Apellido" debajo de "Nombre" (`id="apellido"`, `placeholder="Tu apellido"`, `autoComplete="family-name"`, `required`, `maxLength={120}`); "Nombre" pasa a `autoComplete="given-name"` y `maxLength={120}`; el body del fetch suma `apellido`. La nota "Solo usamos tu nombre y teléfono" (~línea 263) pasa a nombrar el apellido. La réplica chica que llama a `frentePase` (hoy usa `primario`, ~líneas 63-66 y 94-104) pasa a usar `estado`, con `franja: 'banda'`, `nombreCliente: null`, `apellidoCliente: null`. No toques `app/globals.css` (ahí viven los estilos de `.cardface`).
- [ ] **Alta por teléfono** (`altaPorTelefono.ts` + `agregar/actions.ts` + `FormularioAgregarCliente.tsx`): `DatosAltaPorTelefono.apellido?: string`, OPCIONAL. `altaPorTelefono` recorta; vacío o ausente → `null`; más de 120 → `{ ok: false, error: 'El apellido puede tener hasta 120 caracteres.' }` SIN llamar a `registrarCliente`. El formulario: campo "Apellido (opcional)" con `maxLength={120}`, sin `required`; actualizá el comentario que dice "Tres campos". Pruebas: apellido vacío → cliente con `apellido` null (no `''`, que rompería con 23514); 121 caracteres → el mensaje exacto y ningún cliente creado.
...
**Files:** `lib/google/construirRecursos.ts` (+test), `lib/google/heroUrl.ts` (+test), `lib/google/syncObjeto.ts` (+test), `lib/google/linkGuardar.ts` (+test), `app/api/tarjetas/[tarjetaId]/hero.png/route.ts` (solo su comentario de cabecera, que dice "solo sellos con meta"). Depende de la Task 1. **NO toques `construirClase`** (eso es la Task 9, después del deploy A).
...
- [ ] **`heroImage` SIEMPRE que haya `heroImageUrl`, en todos los tipos** (decisión 8 del spec, leé su "Por qué siempre"): el sync hace `patch`, y un hero mandado solo con franja propia dejaría en Android la franja borrada para siempre cuando el dueño la quita.
- [ ] **`textModulesData`**, en este orden y cada uno solo si su dato existe: `{ id: 'nombre_pase', header: 'Tarjeta', body: sobreFranja }`, `{ id: 'estado', header: estado.etiqueta, body: estado.valor }`, `{ id: 'nombre', header: 'NOMBRE', body }`, `{ id: 'apellido', header: 'APELLIDO', body }`. **Mandalo SIEMPRE, aunque quede `[]`**: mismo motivo del patch (un nombre de pase borrado seguiría viéndose).
...
- [ ] **`versionHeroTarjeta(marca, tipoTarjeta, puntos, selloMeta)`** en `heroUrl.ts`: llama a `versionHero` con los puntos y la meta REALES solo si la imagen es la grilla (sellos con meta `> 0` y sin `marca.stripUrl`); en cualquier otro caso con `puntos: 0, selloMeta: null`, porque la franja propia y la banda no cambian al operar. Reemplazá las llamadas a `versionHero` de `syncObjeto.ts` y `linkGuardar.ts` por este ayudante. Pruebas: grilla → dos puntajes dan versiones distintas; franja propia y gift card sin franja → la MISMA. MUTACIÓN: pasar siempre los puntos reales.
- [ ] `syncObjeto.ts` (select ~línea 27) y `linkGuardar.ts` (~línea 32) **no tienen hoy join a `clientes`**: agregá `clientes(nombre, apellido)` y pasá `nombreCliente`, `apellidoCliente` y `stripUrl: marca.stripUrl` a `construirObjeto`. Sus pruebas verifican que el cuerpo mandado lleva los módulos (mockeando `walletClient`). En `linkGuardar.test.ts`, prueba de que el `?v=` del hero del objeto del JWT es el MISMO que el de `syncObjetoTarjeta` para la misma tarjeta (hay un patrón parecido ~líneas 140-148).
- [ ] **Pruebas de `construirRecursos.test.ts`** — hay pruebas que hoy afirman lo contrario y hay que INVERTIRLAS a propósito, no restaurar el código viejo para que pasen: `:257-270` (espera `textModulesData` ausente), `:53` (`barcode` exacto sin `alternateText`), `:127` (puntos sin hero), `:92`/`:100` (hero solo en sellos). Casos nuevos: membresía con nombre de pase y banda; gift card con franja propia (sin `nombre_pase`, con `heroImage`); gift card sin franja (con `heroImage`, con `nombre_pase` si lo tiene); descuento sin estado; cliente sin apellido; `[]` sin datos; `alternateText`. **Mutaciones**: (a) hero solo en `'grilla'`/`'propia'` → falla la gift card sin franja; (b) omitir `textModulesData` cuando está vacío → falla la del `[]`; (c) `'propia'` sin mirar `heroImageUrl`.
- [ ] Actualizá los comentarios que quedan viejos: `construirRecursos.ts` (~62-67 y ~88-96, hablan de `.primario` y de "hero solo en sellos") y la cabecera de `hero.png/route.ts`.
- [ ] Commit.
...
- [ ] **`coincideBusqueda(cliente: { nombre; apellido; telefono } | null, q: string): boolean`**: sale del filtro en memoria de `clientes/page.tsx` (~líneas 33-34, 65-71) SIN cambiar su semántica, y suma el apellido. El placeholder "Nombre o teléfono…" (~línea 133) pasa a "Nombre, apellido o teléfono…". Pruebas: por nombre, por apellido, por teléfono, `q` vacía. MUTACIÓN: sacar el apellido del texto buscado.
...
- [ ] **`objetos --aplicar`**: tarjetas con `google_object_id is not null` (NUNCA `syncObjetosComercio`, que crearía objetos), agrupadas por comercio, en secuencia con `syncObjetoTarjeta`. Resumen por comercio: ok / fallos con el error. Separá los fallos ESTRUCTURALES, que reintentar no arregla (`'El comercio no tiene Google Wallet habilitado.'` de `syncObjeto.ts`, comercio sin logo de `syncClase.ts`; leé los mensajes reales), de los de Google. **Exit 1 si hubo algún fallo no estructural** (el deploy B depende de 0); los estructurales se listan aparte para que el controlador decida.
- [ ] **`clases --aplicar`**: `syncClaseComercio` para comercios con `google_class_id`; `syncClasePrograma` SOLO para programas con `google_class_id`, leyendo su resultado (`{ ok, classId }` / `{ ok: false, error }`). Resumen con la misma separación de estructurales y exit 1 si hubo fallos no estructurales. Imprimí al empezar un recordatorio de que esta fase se corre desde el commit del deploy B.
...
- [ ] `construirClase` suma `classTemplateInfo: { cardTemplateOverride: { cardRowTemplateInfos: [fila1, fila2] } }` con dos filas `{ twoItems: { startItem, endItem } }`, cada ítem `{ firstValue: { fields: [{ fieldPath }] } }`:
...
- [ ] Documento de estado (`docs/superpowers/ESTADO-Y-PLAN-2026-07-28.md`): sección nueva con lo que entró, lo no obvio y lo pendiente (fase objetos, deploy B, fase clases, QA en teléfono). Anotá TODOS los caminos que re-sincronizan la clase solos (registro, `linkGuardar`, guardado de marca, `admin/comercios/actions.ts`, `sucursales/actions.ts`, `campanasVencidas.ts`): son el motivo de los dos deploys.
