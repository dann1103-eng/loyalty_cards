...
// más profundo gana sobre el de la raíz). Ver docs/superpowers/specs/2026-09-23-onboarding-manifest-
// monto-resena-design.md, sección 1.
...
// propio del comercio o del admin que dibujar ahí.
...
// Mismo fondo/tema que app/manifest.ts (#e7e6f0, el claro por defecto). Se mueven JUNTOS a
// propósito, no se importan de ahí: app/manifest.ts es un archivo especial de Next (solo puede
// tener un default export) y la regla del proyecto es no tocarlo.
const FONDO = '#e7e6f0';
...
export function manifiestoComercio(): MetadataRoute.Manifest {
  return {
    ...
    id: '/comercio/panel',
    // CON barra final: todo lo que cuelga de /comercio/ es del panel, incluidas las pantallas sin
    // sesión (login, activar, clave, elegir, suspendida) a las que el proxy redirige. Ver la
    // invariante start_url.startsWith(scope) en manifiestos.test.ts.
    scope: '/comercio/',
    display: 'standalone',
...
export function manifiestoAdmin(): MetadataRoute.Manifest {
  return {
    ...
    // start_url fuera del scope hace que el navegador descarte el scope entero. No hace falta que
    // exista una página en /admin a secas para que esto valga: el scope solo compara el prefijo.
    scope: '/admin',
    display: 'standalone',
    background_color: FONDO,
    theme_color: FONDO,
